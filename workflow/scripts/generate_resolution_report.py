...
Stage 5a — Generate Resolution-Only Summary Report
===================================================
Produces results/resolution_report.html covering resolution
statistics without requiring downloaded assemblies or extracted
sequences.  Can be run immediately after merge_resolved.
"""
...
<head>
  <meta charset="UTF-8">
  <title>RNA Flanking Pipeline — Resolution Report</title>
  <style>
...
<body>
<header>
  <h1>🧬 RNA Flanking Sequence Pipeline — Resolution Report</h1>
  <p>Generated: {now} &nbsp;|&nbsp;
     Upstream: {UPSTREAM} bp &nbsp;|&nbsp; Downstream: {DOWNSTREAM} bp &nbsp;|&nbsp;
     Ensembl release: {RELEASE} &nbsp;|&nbsp;
     <em>Resolution stage only — sequences not yet extracted</em></p>
</header>
<main>

  {report_intro_html(
      report_title="Resolution report",
      summary="Use this report to review classification coverage, unresolved IDs, ambiguous mappings, and prefix issues before running download or extraction.",
      related_href="report.html",
      related_label="full report",
  )}
  ...
  <div class="info-box">
    Resolution complete. Run <code>snakemake results/report.html</code> (or the full
    pipeline) to download assemblies, extract flanking sequences, and generate the
    final report including sequence counts.
  </div>

  <h2>Resolution by Resolver</h2>
  <table class="data-table">
    <thead><tr><th>Resolver</th><th>db_source</th><th>Input IDs</th><th>Resolved</th><th>Unresolved</th><th>Rate</th><th>Method</th></tr></thead>
