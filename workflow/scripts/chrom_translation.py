        ...
        for line in fh:
            ...
            for alias in aliases:
                if alias and alias != "na":
                    xlate[alias] = refseq
                    xlate[alias.lower()] = refseq  # case-insensitive lookup
    return xlate
...
def resolve_chrom_key(chrom: str, xlate: dict, chrom_lengths) -> Optional[str]:
    ...
    report map (friendly -> RefSeq) is consulted first, then the raw name is
    ...
    for cand in candidates:
        mapped = xlate.get(cand) or xlate.get(cand.lower())
        if mapped and mapped in chrom_lengths:
            return mapped
