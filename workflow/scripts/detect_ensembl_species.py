...
# ── Load data ─────────────────────────────────────────────────
log.info("detect_ensembl_species: auto-detecting species from Ensembl transcript ID prefixes")
...
if df_ensembl.empty:
    ...
    pd.DataFrame(columns=["transcript_id", "prefix", "species", "build"]) \
        .to_csv(out_map, sep="\t", index=False)
    pd.DataFrame(columns=["prefix", "example_id", "n_transcripts", "action_required"]) \
...
for pfx, info in overrides.items():
    ...
    log.info(f"  Config override applied: {pfx.upper()} → {info['species']} ({info['build']})")
...
df_unk = pd.DataFrame(unk_rows, columns=["prefix", "example_id", "n_transcripts", "action_required"])
...
if unknown:
    ...
    action_file = Path(out_unk).with_suffix("").with_suffix("") \
        .parent / "ensembl_unknown_prefixes_ACTION_REQUIRED.txt"
