...
def fix_esearch_query_BEFORE():
    ...
    handle = with_retry(
        Entrez.esearch,
        db="nucleotide",
        term=" OR ".join(f"{acc}[ACCN]" for acc in batch),  # ← Problem: [ACCN] excludes suppressed
        retmax=len(batch) * 5,
        label="ncbi_esearch",
    )
...
def resolve_via_efetch(accession: str) -> Optional[dict]:
    ...
    try:
            ...
            if feat.get('GBFeature_key') == 'gene':
                ...
                for qual in qualifiers:
                    ...
                    if qual_name == 'db_xref' and 'GeneID:' in qual_value:
                        ...
                        gene_id = qual_value.split('GeneID:')[1].split(',')[0].split(';')[0].strip()
        ...
        if not gene_id:
            log.warning(f"  {accession}: GenBank record has no gene feature with GeneID")
            return None
        
        log.info(f"  {accession}: extracted GeneID={gene_id} from GenBank feature table")
        ...
        doc = gene_summaries.get('DocumentSummarySet', {}).get('DocumentSummary', [{}])[0]
...
def integration_point_in_resolve_ncbi_batch():
    ...
    if not gene_ids_for_tid:
        ...
        if fallback_result:
            ...
            log.info(f"  ✓ {tid} resolved via efetch fallback → gene {fallback_result['gene_id']}")
        else:
