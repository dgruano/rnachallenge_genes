    ...
    try:
        ...
        for i, record_text in enumerate(records):
            ...
            if gene_info:
                ...
                print(f"  [{i+1}] {gene_info['accession']} → Gene {gene_info['gene_id']} ({gene_info['gene_symbol']})")
            else:
    ...
    try:
        with gzip.open(local_file, 'rt') as f:
            for line in f:
                ...
                if lines_processed % 1000000 == 0:
                    print(f"    Processed {lines_processed:,} lines, index size: {len(index):,}")
        
        print(f"  ✓ Index built: {len(index):,} accessions from {lines_processed:,} lines")
        ...
        for tid in ids:
            if tid in index:
                ...
                print(f"    ✓ {tid} → Gene {gene_data['gene_id']} (status: {gene_data['status']})")
            else:
                # Try base ID
...
def test_efetch_formats(test_id: str) -> Dict:
    ...
    for rettype, retmode, description in formats_to_test:
        ...
        try:
            ...
            print(f"    ✓ Gene ID found: {has_gene_id} ({gene_id if gene_id else 'N/A'})")
...
def estimate_performance_9k_ids():
    ...
    for est in estimates:
        ...
        total = f"{est['total_time_minutes']:.1f} min" if est['total_time_minutes'] < 120 else f"{est['total_time_minutes']/60:.1f} hr"
        speedup = est["speedup"]
...
def print_summary_and_recommendations():
    ...
    gene2refseq_success = RESULTS.get("bulk_files", {}).get("gene2refseq", {}).get("success", False)
    gene2accession_success = RESULTS.get("bulk_files", {}).get("gene2accession", {}).get("success", False)
